...
function isConflict(actions) {
  return (
    (typeof actions.shift === "undefined" ? 0 : 1) +
      (typeof actions.reduce !== "undefined" ? actions.reduce.length : 0) >
    1
  );
}
      ...
      <thead>
        <tr>
          ...
          {symbolInfo.nonterminalOrder.map(function (symbol, index) {
            return (
              <th key={"nt" + index}>{formatSymbol(symbol, symbolInfo)}</th>
            );
          })}
        </tr>
      </thead>
              ...
              {symbolInfo.terminalOrder.concat(END).map(function (s, index) {
                if (typeof state[s] === "undefined") {
                  ...
                  return (
                    <td
                      key={"t" + index}
                      className={isConflict(state[s]) ? "conflict" : ""}
                    >
                      <ul>{actions}</ul>
                    </td>
                  );
