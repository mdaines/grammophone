...
describe("nullable", function () {
  ...
  it("returns the expected results from Louden fixtures", function () {
    assert.deepStrictEqual(
      new Grammar(Fixtures.expressions).calculations.nullable,
      new Set()
    );
    assert.deepStrictEqual(
      new Grammar(Fixtures.ifelse).calculations.nullable,
      new Set(["else-part"])
    );
    assert.deepStrictEqual(
      new Grammar(Fixtures.statements).calculations.nullable,
      new Set(["stmt-seq'"])
    );
  });
});
